...
namespace Model
{
    public class 弓骑 : Active
    {
        ...
        public override async Task Execute(List<Player> dests, List<Card> cards, string other)
        {
            ...
            TimerTask.Instance.Hint = "弃置一名其他角色的一张牌";
            TimerTask.Instance.IsValidDest = x => x != Src;
            if (await TimerTask.Instance.Run(Src, 0, 1))
            {
    ...
    public class 解烦 : Active, Ultimate
    {
        ...
        public override async Task Execute(List<Player> dests, List<Card> cards, string other)
        {
            ...
            while (true)
            {
                ...
                {
                    TimerTask.Instance.Hint = "弃置一张武器牌，或令该角色摸一张牌";
                    TimerTask.Instance.IsValidCard = x => x is Weapon;
