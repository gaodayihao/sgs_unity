...
namespace Model
{
    public class 千驹 : Triggered
    {
        ...
        public override void OnEnable()
        {
            ...
            Src.playerEvents.recover.AddEvent(Src, Execute);
        }
        ...
        public override void OnDisable()
        {
            ...
            Src.playerEvents.recover.RemoveEvent(Src, Execute);
            Src.DstSub -= offset;
        }
    ...
    public class 倾袭 : Triggered
    {
        ...
        public async Task Execute(Card card)
        {
            ...
            foreach (var i in card.Dests)
            {
                ...
                TimerTask.Instance.Hint = "弃置" + count + "张手牌，然后弃置其武器牌，或令此牌伤害+1";
                TimerTask.Instance.IsValidCard = x => i.HandCards.Contains(x);
                bool result = await TimerTask.Instance.Run(i, count, 0);
                ...
                if (result)
                {
                    await new Discard(i, TimerTask.Instance.Cards).Execute();
